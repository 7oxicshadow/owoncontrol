...
char video_trigger_data[VID_TRIG_DATA_SIZE] = {0x3a,0x4d,0x00,0x00,0x00,0x1b,0x4d,0x54,0x52,0x73,0x00,0x76,0x02,0x00,0x4d,
                                               ...
                                               0x00,0x00,0x01};
/*
                            byte[10] & byte[18] & byte[26] ---------- (CH1 = 0x00 CH2 = 0x01)
                            byte[13] ---------- (NTSC = 0x00 PAL = 0x01 SECAM = 0x02)
                            byte[21] ---------- (LINE = 0x00 FIELD = 0x01 ODD = 0x02 EVEN = 0x03)
*/
...
/*
                            byte[10] & byte[18] & byte[30] ---------- (CH1 = 0x00 CH2 = 0x01)
                            byte[13] ---------- (NTSC = 0x00 PAL = 0x01 SECAM = 0x02)
                            byte[22] & byte[23] & byte[24] & byte[25] = (value between 1 and 625) (note: 525 max ntsc mode)
*/
